...
function Monster(config) {
  ...
  this.species = config.species || "Unkonwn";
  ...
  this.size = {
    ...
    crown: {
      ...
      lare: config.size?.crown?.lare || 1.1,
    },
  };
  this.stats = {
    health: {
      ...
      modifier: {
        ...
        masterRank: config.stats?.health?.modifier?.Monster || 3.2,
      },
    },
    ...
    staggerThershold: config.stats?.staggerThershold || 150,
  };
  ...
  this.breakableParts = config.breakableParts || [];
  this.severableParts = config.severableParts || [];

  this.behavior = {
    aggressionLevel: config.behavior?.aggressionLevel || 1,
    temitorialRange: config.behavior?.temitorialRange || 50,
    fleeTrehold: config.behavior?.fleeTrehold || 0.15,
    naturalPreys: config.behavior?.naturalPreys || [],
    naturalPredators: config.behavior?.naturalPredators || [],
